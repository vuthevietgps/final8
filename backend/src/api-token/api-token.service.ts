...
class GoogleValidator implements ProviderValidator {
  async validate(rawToken: string): Promise<TokenValidationResult> {
    // Google Ads sử dụng refresh token, không thể validate trực tiếp như access token
    // Chỉ kiểm tra format cơ bản
    if (!rawToken || rawToken.length < 20) {
      return { status: 'invalid', message: 'Token quá ngắn hoặc không hợp lệ' };
    }
    
    // Google refresh token thường bắt đầu với "1//"
    if (rawToken.startsWith('1//')) {
      return { 
        status: 'valid', 
        message: 'Google refresh token có format hợp lệ (cần test với API để xác nhận đầy đủ)', 
    ...
    return { 
      status: 'valid', 
      message: 'Token được chấp nhận (chưa xác thực với Google API)', 
      scopes: [] 
    };
  }
}
  ...
  async validate(rawToken: string): Promise<TokenValidationResult> {
    try {
      ...
      if (response.ok && data.code === 0) {
        ...
        return { 
          status: 'valid', 
          message: `TikTok token hợp lệ (${advertisers.length} advertiser(s))`, 
          scopes: ['ad_data_read'] 
        };
      } else if (data.message) {
        return { status: 'invalid', message: `TikTok API error: ${data.message}` };
      } else {
        return { status: 'invalid', message: 'TikTok token không hợp lệ' };
      }
    } catch (error) {
      return { 
        status: 'invalid', 
        message: `Lỗi kết nối TikTok API: ${error instanceof Error ? error.message : 'Unknown error'}` 
      };
    }
  }
}
...
export class ApiTokenService {
  ...
  async testGoogleAdsConnection(params: {
    ...
    try {
      ...
      if (!tokenRes?.token) {
        return { ok: false, message: 'Không thể lấy access token từ refresh token', error: 'Invalid refresh token' };
      }

      // Test API call
      const customerId = params.customerId.replace(/[^0-9]/g, '');
      const url = `https://googleads.googleapis.com/v15/customers/${customerId}`;
      ...
      if (response.ok) {
        ...
        return {
          ok: true,
          message: `Kết nối thành công với Google Ads`,
          account: {
            id: customerId,
        ...
        return {
          ok: false,
          message: `Lỗi Google Ads API: ${response.status}`,
      ...
      return {
        ok: false,
        message: 'Lỗi kết nối Google Ads',
  ...
  async testTikTokConnection(params: {
    ...
    try {
      ...
      if (body.code === 0 && body.data?.list?.length > 0) {
        ...
        return {
          ok: true,
          message: `Kết nối thành công với TikTok Ads`,
          account: {
        ...
        return {
          ok: false,
          message: `Lỗi TikTok API: ${body.message || 'Unknown'}`,
          error: body.message || `Code: ${body.code}`
        };
      }
    } catch (err: any) {
      return {
  ...
  /**
   * Lưu Google Ads settings vào database (collection settings hoặc dùng token đặc biệt)
   */
  async saveGoogleAdsSettings(params: {
    ...
    try {
      // Tìm hoặc tạo token đặc biệt cho Google Ads settings
      ...
      const tokenData = {
        ...
        adAccountId: params.loginCustomerId,
      ...
      return { ok: true, message: 'Đã lưu cấu hình Google Ads' };
    } catch (err: any) {
      return { ok: false, message: err?.message || 'Lỗi lưu settings' };
    }
  }
  ...
  async saveTikTokSettings(params: {
    ...
    try {
      ...
      const tokenData = {
        ...
        tokenEnc: encryptToken(params.accessToken),
      ...
      return { ok: true, message: 'Đã lưu cấu hình TikTok Ads' };
    } catch (err: any) {
      return { ok: false, message: err?.message || 'Lỗi lưu settings' };
    }
  }
  ...
  /**
   * Lấy settings hiện tại (masked)
   */
  async getAdsSettings(): Promise<{
    facebook: { configured: boolean; tokenCount: number };
    google: { configured: boolean; clientId?: string; hasRefreshToken: boolean; developerToken?: string };
    ...
    const googleSettings = await this.model.findOne({ provider: 'google', name: 'Google Ads System Settings' });
    const tiktokSettings = await this.model.findOne({ provider: 'tiktok', name: 'TikTok Ads System Settings' });

    let googleConfig: any = { configured: false, hasRefreshToken: false };
    if (googleSettings) {
      try {
        const notes = JSON.parse(googleSettings.notes || '{}');
        googleConfig = {
          configured: true,
          clientId: notes.clientId ? notes.clientId.slice(0, 20) + '...' : undefined,
          hasRefreshToken: !!googleSettings.token || !!googleSettings.tokenEnc,
          developerToken: notes.developerToken ? notes.developerToken.slice(0, 10) + '...' : undefined,
          loginCustomerId: notes.loginCustomerId
        };
      } catch {}
    }

    let tiktokConfig: any = { configured: false, hasAccessToken: false };
    if (tiktokSettings) {
      tiktokConfig = {
        configured: true,
        hasAccessToken: !!tiktokSettings.token || !!tiktokSettings.tokenEnc
      };
    }
