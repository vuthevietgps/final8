...
export class OtherCostController {
  ...
  /**
   * GET /other-cost/summary/cashflow
   * Tổng hợp chi phí vận hành cho Financial Control (AP)
   */
  @Get('summary/cashflow')
  @RequirePermissions('other-costs')
  getCashflowSummary(@Query('windowDays') windowDays?: string) {
    return this.otherCostService.getCashflowSummary(windowDays ? parseInt(windowDays, 10) : 14);
  }
