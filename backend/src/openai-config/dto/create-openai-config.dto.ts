...
export class CreateOpenAIConfigDto {
  ...
  @IsOptional() @IsEnum(['admin-assistant','customer-chatbot','general']) purpose?: 'admin-assistant' | 'customer-chatbot' | 'general';
  @IsString() model: string;
  @IsString() apiKey: string;
  @IsString() systemPrompt: string;
  @IsOptional() @Type(() => Number) @IsNumber() @Min(1) @Max(32000) maxTokens?: number;
  @IsOptional() @Type(() => Number) @IsNumber() @Min(0) @Max(2) temperature?: number;
  @IsOptional() @IsEnum(['none','low','medium','high','xhigh']) reasoningEffort?: 'none' | 'low' | 'medium' | 'high' | 'xhigh';
