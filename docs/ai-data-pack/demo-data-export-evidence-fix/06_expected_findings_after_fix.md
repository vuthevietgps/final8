...
Prompt 32 rerun result: `12/12` expected findings present.

| Expected key | Status | Matched pattern |
|---|---|---|
...
| `high_sales_late_payment_agent` | `present` | `high_sales_late_payment_agent` |
| `return_rate_above_policy` | `present` | `return_rate_above_policy_for_single_offer` |
| `google_ads_mapping_gap` | `present` | `ads_account_to_campaign` |
| `inventory_movement_gap` | `present` | `inventory_movement_without_matching_purchase_order` |

The three findings missing after Prompt 31 are now present in the Director JSON export.
