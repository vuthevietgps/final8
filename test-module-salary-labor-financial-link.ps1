...
<#
  =====================================================================================
  TEST-MODULE-SALARY-LABOR-FINANCIAL-LINK.ps1
  =====================================================================================
...
try {
  ...
  Write-Step "4.1" "Set Financial Control CommittedWindowDays = 400 (for robust coverage)"
  $fcPatch = Safe-Request -Method PATCH -Uri "$BaseUrl/financial-control/config" -Headers $headers -Body (To-JsonBody @{ CommittedWindowDays = 400 }) -Label "PatchFcConfig"
  if ($fcPatch -and $fcPatch.CommittedWindowDays -eq 400) {
    Write-Pass "CommittedWindowDays patched to 400"
  } else {
  ...
  $laborBeforePay = Safe-Request -Method GET -Uri "$BaseUrl/labor-cost1/summary/cashflow?windowDays=400" -Headers $headers -Label "LaborSummaryBeforePay"
  ...
  $laborAfterPay = Safe-Request -Method GET -Uri "$BaseUrl/labor-cost1/summary/cashflow?windowDays=400" -Headers $headers -Label "LaborSummaryAfterPay"
  ...
  if (($fcBurnLaborAfter - $fcBurnLaborBefore) -ge ($payAmount - 2)) {
    Write-Pass "FC monthlyBurnBreakdown.laborCore increased by payment amount (delta=$($fcBurnLaborAfter - $fcBurnLaborBefore))"
  } else {
    Write-Fail "FC monthlyBurnBreakdown.laborCore did not reflect payment delta (delta=$($fcBurnLaborAfter - $fcBurnLaborBefore))"
  }
