...
<#
    =====================================================================================
    E2E.ORDER-FINANCE-IMPACT.PS1 v2
    =====================================================================================
...
if (!$adGroupId -and $fanpageId -and $catId -and $extAgentId -and $adAccountId) {
    $agBody = (@{
        ...
        fanpageId = $fanpageId; productCategoryId = $catId; selectedProducts = @($productId)
        agentId = $extAgentId; adAccountId = $adAccountId
        isActive = $true
    } | ConvertTo-Json)
...
if ($extAgentId -and $productId) {
    $aqBody = (@{
        agentId = $extAgentId; productId = $productId; unitPrice = 80000
        status = "approved"; validFrom = (Get-Date).ToString("yyyy-MM-dd")
        validUntil = (Get-Date).AddMonths(3).ToString("yyyy-MM-dd")
        notes = "FC impact test quote"
    } | ConvertTo-Json)
...
if ($order1Id) {
    ...
    if ($order1Del) {
        ...
        # External-agent commission follows COD - (agentQuote * qty).
        # Shipping remains a company-side cost and should not reduce agent payout.
        $expectedAP = $cod - ($aq * $qty)
...
if ($order2Id) {
    ...
    if ($order2Ret) {
        ...
        if ($sq2 -gt 0) {
            $supplierIsReturnable = if ($null -ne $order2Ret.supplierIsReturnableSnapshot) { [bool]$order2Ret.supplierIsReturnableSnapshot } else { $true }
            $supplierCostOnReturn = if ($supplierIsReturnable) { 0 } else { ($sq2 * $qty2) }
            $expectedRetSP = 0 - $supplierCostOnReturn - $ship2 - $ret2
            Assert-Equal "Return supplierPaidAmount" $expectedRetSP ([double]$order2Ret.supplierPaidAmount)
        }

        # For return: external-agent clawback = 0 - (aq * qty)
        if ($aq2 -gt 0) {
            $expectedRetAP = 0 - ($aq2 * $qty2)
            Assert-Equal "Return agentPaidAmount" $expectedRetAP ([double]$order2Ret.agentPaidAmount)
        }

        # grossProfit for return uses the same universal formula:
        # effectiveCod - supplierCost - shippingFee - returnFee - agentCommission
        $supplierIsReturnable2 = if ($null -ne $order2Ret.supplierIsReturnableSnapshot) { [bool]$order2Ret.supplierIsReturnableSnapshot } else { $true }
        $supplierCostOnReturn2 = if ($supplierIsReturnable2) { 0 } else { ($sq2 * $qty2) }
        $expectedRetGrossProfit = 0 - $supplierCostOnReturn2 - $ship2 - $ret2 - $expectedRetAP
        Assert-Equal "Return grossProfit" $expectedRetGrossProfit ([double]$order2Ret.grossProfit)
...
if ($fcAfterCost -and $cost1Id) {
    ...
    Assert-NumberChanged "monthlyBurn (after unpaid cost)" $burnBefore $fcAfterCost.monthlyBurn "increase"
...
if ($cost1Id) {
    ...
    if ($costConfirmed) {
        ...
        if ($fcAfterConfirm) {
            ...
            Assert-NumberChanged "monthlyBurn (after confirm)" $fcAfterCost.monthlyBurn $fcAfterConfirm.monthlyBurn "decrease"
...
if ($loanId) {
    $payOpt = Safe-Request -Method GET -Uri "$BaseUrl/loan-management/loans/$loanId/payment-options" -Headers $h -Label "PaymentOptions"
    $availableBank = if ($payOpt -and $payOpt.sources -and $payOpt.sources.bankBalance) { [double]$payOpt.sources.bankBalance.available } else { 0 }
    $canPayFromBank = if ($payOpt -and $payOpt.sources -and $payOpt.sources.bankBalance) { [bool]$payOpt.sources.bankBalance.canUse } else { $false }
    if ($canPayFromBank -and $availableBank -ge 1) {
        $repayAmount = [math]::Min(1000000, [math]::Floor($availableBank))
        $repayBody = (@{ paymentType = "principal"; amount = $repayAmount; source = "bank_balance"; notes = "FC test repay"; paymentDate = (Get-Date).ToString("yyyy-MM-dd") } | ConvertTo-Json)
        $repay = Safe-Request -Method POST -Uri "$BaseUrl/loan-management/loans/$loanId/pay" -Headers $h -Body $repayBody -Label "Repay"
        if ($repay) {
            Write-Pass "Repaid principal from bank balance: $repayAmount"
            Wait-CacheExpire
        ...
        Write-Pass "Repay correctly blocked when freeCash is insufficient: available=$availableBank"
        $fcAfterRepay = $fcAfterDisb
        if ($payOpt) {
            Write-Info "Loan payment options: canPayFromBank=$canPayFromBank, availableBank=$availableBank"
        }
    }
}
