...
try {
  ...
  Write-Step "1.5" "Create Product for single-product ad group rule"
  $productBody = @{
    name = "Product NP-$ts"
  ...
  Write-Step "1.6" "Create Ad Account"
  $adAccBody = @{
  ...
  Write-Step "1.7" "Create Ad Group"
  $adGroupBody = @{
    name = "AdGroup NP-$ts"
  ...
  Write-Step "1.8" "Create Advertising Cost = $adCostAmount"
  $adCostBody = @{
  ...
  Write-Step "1.9" "Create Labor Cost1 = $laborCostAmount (1 hour)"
  $laborBody = @{
    date = $testDateFull
    userId = $agentUserId
    startTime = "08:00"
  ...
  Write-Step "1.10" "Create Other Cost = $otherCostAmount"
  $otherBody = @{
    date = $testDateFull
  ...
  $order1Body = @{
    ...
    codAmount = $o1Cod
    supplierAppliedPrice = $o1SupplierQuote
  ...
  $order2Body = @{
    ...
    orderDate = $testDateFull
    codAmount = $o2Cod
    supplierAppliedPrice = $o2SupplierQuote
  ...
  if ($createdProductId) { [void](Safe-Request -Method DELETE -Uri "$BaseUrl/products/$createdProductId" -Headers $h -Label "DeleteProduct") ; Write-Pass "Delete product requested" }
