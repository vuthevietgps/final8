...
$policyList = if ($policies -is [array]) { $policies } elseif ($policies.data) { $policies.data } elseif ($policies -ne $null) { @($policies) } else { @() }
Write-Pass "Policies listed: $($policyList.Count)"
...
if ($activePolicy -eq $null) {
    Write-Info "No active policy found, creating temporary policy for regression"
    $policyBody = @{
        name = "QA Policy $ts"
        description = "Temporary policy for module.ads-alerts-kpi"
        reinvestmentRatio = 45
        safetyReserveRatio = 25
        personalIncomeRatio = 20
        longTermAssetRatio = 10
        isActive = $true
        notes = "Temporary regression policy"
    } | ConvertTo-Json
    $activePolicy = Safe-Request -Method POST -Uri "$BaseUrl/capital-allocation/policies" -Headers $h -Body $policyBody -Label "CreatePolicy"
    if ($activePolicy -and $activePolicy._id) {
        $createdPolicyId = [string]$activePolicy._id
        Write-Pass "Temporary active policy created"
    } else {
        Write-Fail "Active policy failed"
    }
} else {
    Write-Pass "Active policy OK"
}
...
$snapList = @(
    if ($snaps -is [array]) { $snaps }
    elseif ($snaps -and $snaps.data) { $snaps.data }
    elseif ($null -eq $snaps) { @() }
    else { @($snaps) }
)
if ($null -ne $snapList) { Write-Pass "Snapshots listed: $($snapList.Count)" } else { Write-Fail "List snapshots failed" }
...
Write-Step "3.8" "Cleanup temporary policy"
if ($createdPolicyId) {
    $deactivatePolicy = Safe-Request -Method PATCH -Uri "$BaseUrl/capital-allocation/policies/$createdPolicyId" -Headers $h -Body '{"isActive":false}' -Label "DeactivatePolicy"
    $cleanupPolicy = Safe-Request -Method DELETE -Uri "$BaseUrl/capital-allocation/policies/$createdPolicyId" -Headers $h -Label "DeletePolicy"
    if ($deactivatePolicy -ne $null -and $cleanupPolicy -ne $null) { Write-Pass "Temporary policy deleted" } else { Write-Fail "Temporary policy cleanup failed" }
} else {
    Write-Info "No temporary policy to cleanup"
}
