...
function Expect-Fail {
    ...
    try {
        ...
        $st = $_.Exception.Response.StatusCode.value__
        return ($st -eq $ExpectedStatus)
    }
}
function Get-UserByEmail {
    param([string]$Email, [hashtable]$Headers)
    $encodedEmail = [uri]::EscapeDataString($Email)
    return (Safe-Request -Method GET -Uri "$BaseUrl/users/email/$encodedEmail" -Headers $Headers -Label "GetUserByEmail-$Email")
}

$script:passCount = 0; $script:failCount = 0; $script:failDetails = @()
$ts = Get-Date -Format "yyyyMMdd-HHmmss"
$supplierEmail = "test-supplier-sc@test.com"
$agentEmail = "test-agent-sc@test.com"
...
$supplier = Get-UserByEmail -Email $supplierEmail -Headers $h
$agent = Get-UserByEmail -Email $agentEmail -Headers $h
if ($users) {
    $userList = if ($users.data) { $users.data } elseif ($users -is [array]) { $users } else { @($users) }
    $supplier = $userList | Where-Object { $_.email -eq $supplierEmail } | Select-Object -First 1
    $agent = $userList | Where-Object { $_.email -eq $agentEmail } | Select-Object -First 1
