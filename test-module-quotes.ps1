...
$product = $null
if ($products -is [array] -and $products.Count -gt 0) { $product = $products[0] }
elseif ($products.data -and $products.data.Count -gt 0) { $product = $products.data[0] }

if (-not $product) {
    Write-Info "No product found, creating..."
    $catBody = '{"name":"Test Cat Quotes","isActive":true}'
    $cat = Safe-Request -Method POST -Uri "$BaseUrl/product-category" -Headers $h -Body $catBody -Label "CreateCat"
    $catId = if ($cat._id) { $cat._id } elseif ($cat.id) { $cat.id } else { "" }
    if ($catId) {
        $prodBody = (@{ name = "Test Product Quotes"; categoryId = $catId } | ConvertTo-Json)
...
if ($qList -ne $null) {
    $count = if ($qList -is [array]) { $qList.Count } else { 1 }
...
if ($qByAgent -ne $null) {
    $count = if ($qByAgent -is [array]) { $qByAgent.Count } else { 1 }
...
if ($qByProd -ne $null) {
    $count = if ($qByProd -is [array]) { $qByProd.Count } else { 1 }
...
if ($sqList -ne $null) {
    $count = if ($sqList -is [array]) { $sqList.Count } elseif ($sqList.data) { $sqList.data.Count } else { 1 }
...
if ($sqHist -ne $null) {
    $count = if ($sqHist -is [array]) { $sqHist.Count } else { 1 }
...
if ($sqBySup -ne $null) {
    $count = if ($sqBySup -is [array]) { $sqBySup.Count } else { 1 }
