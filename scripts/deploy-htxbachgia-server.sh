...
#   ./deploy-htxbachgia.sh version14.0       # positional tag
#   ./deploy-htxbachgia.sh --tag version14.0 # explicit tag
...
# Accept optional tag argument, default to version14.0 (current tested version)
...
if [ -z "$EXPECT_BACKEND_DIGEST" ] && [ -z "$EXPECT_FRONTEND_DIGEST" ]; then
  if [ "$TAG_IN" = "version14.0" ]; then
    echo "🔒 No predefined digests for version14.0; please verify manually or pass --expect-* flags"
...
if [ "$TAG_IN" = "version14.0" ]; then
  echo "📝 Changes in this deployment (v14.0):"
  echo "  - Bump UI badge to version14.0"
  echo "  - Build/Deploy: images rebuilt --no-cache and tagged :backend-version14.0, :frontend-version14.0"
elif [ "$TAG_IN" = "version12.0" ]; then
