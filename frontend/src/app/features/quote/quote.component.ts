...
export class QuoteComponent implements OnInit {
  ...
  private showModal() {
    if (!this.quoteModal?.nativeElement) return;
    const bs = (window as any)?.bootstrap;
    if (bs?.Modal) {
      const modal = new bs.Modal(this.quoteModal.nativeElement);
      modal.show();
      return;
    }
    // Fallback: simple class toggle to avoid runtime crash when bootstrap JS is not loaded
    this.quoteModal.nativeElement.classList.add('show');
    this.quoteModal.nativeElement.style.display = 'block';
  }
  ...
  private hideModal() {
    ...
    const bs = (window as any)?.bootstrap;
    if (bs?.Modal) {
      const modal = bs.Modal.getInstance(this.quoteModal.nativeElement);
      if (modal) modal.hide();
      return;
    }
    // Fallback clear
    this.quoteModal.nativeElement.classList.remove('show');
    this.quoteModal.nativeElement.style.display = 'none';
  }
}
