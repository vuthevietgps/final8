...
<#
    =====================================================================================
    TEST-MODULE-REPORTS-PRODUCTS-CONFIG.ps1
    =====================================================================================
...
if ($catSeed -ne $null) { Write-Pass "Category seed OK" } else { Write-Fail "Category seed failed" }
...
if ($prodSeed -ne $null) { Write-Pass "Product seed OK" } else { Write-Fail "Product seed failed" }
...
if ($prodCatId) {
    ...
    if ($prodByCat -ne $null) { Write-Pass "Products by category OK" } else { Write-Fail "Products by category failed" }
}
...
$pList = if ($allProds -is [array]) { $allProds } elseif ($allProds.data) { $allProds.data } else { @($allProds) }
$pendProductId = if ($pList.Count -gt 0) { $pList[0]._id } else { $null }
    ...
    if ($rr1Id) {
        ...
        if ($rrDetail -and $rrDetail.items) {
            foreach ($item in $rrDetail.items) {
                $itemId = if ($item._id) { $item._id } elseif ($item.itemId) { $item.itemId } else { $item.productId }
                $rrItems += @{ itemId = $itemId; decision = "restock" }
            }
        }
        if ($rrItems.Count -eq 0) {
            # Fallback: use productId as itemId
            $rrItems = @(@{ itemId = $retProductId; decision = "restock" })
        }
        $rrResBody = @{
            items = $rrItems
...
$salConfig = Safe-Request -Method GET -Uri "$BaseUrl/salary-config" -Headers $h -Label "SalaryConfig"
if ($salConfig -ne $null) { Write-Pass "Salary config OK" } else { Write-Fail "Salary config failed" }
